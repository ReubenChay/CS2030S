...
class Rest extends Event {
    ...
    Server updateServer(Server server) {
        return new Server(server.getIdentity(), false, server.getMaxQueue(), server.getQueueList(),
                server.getDoneServingTime() + this.restTime, server.getSupplier());
    }

        @Override
    Server updateSelfCheckout(Server server) {
        return new SelfCheckout(server.getIdentity(), false, server.getMaxQueue(), server.getQueueList(),
    ...
    public String toString() {
        return "";
        //        return String.format("%.3f server %d rests for %.3f\n", super.getEventTime(), this.getServerIdentity(), this.restTime);
    }
