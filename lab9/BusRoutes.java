...
class BusRoutes {
    ...
    public CompletableFuture<String> description() {
        String result = "Search for: " + this.stop + " <-> " + name + ":\n"
            + "From " + this.stop + "\n";
        CompletableFuture<String> out = CompletableFuture.supplyAsync(() -> result);

        for (BusService service: services.keySet()) {
            CompletableFuture<Set<BusStop>> stops = services.get(service);
            out = out.thenCombine(describeService(service, stops),
                (x,y) -> x + y);
        }
        return out;
    }
    ...
    public CompletableFuture<String> describeService(
        BusService service, CompletableFuture<Set<BusStop>> stops) {
        CompletableFuture<String> out = stops.thenApply(x -> {
            if (x.isEmpty()) {
                return "";
            } else {
                return x.stream()
