...
class Wait extends Event {
    ...
    Server updateServer(Server server) {
        ...
        /**
          ...
          this.serverAvailableTime + super.getCustomerServiceTime(), server.getRestingThreshold(),
          server.getSupplier(), server.getRestProbSupplier());
          */
        return new Server(this.server.getIdentity(), false, server.getMaxQueue(), newList,
                this.serverAvailableTime, server.getRestingThreshold(),
                server.getSupplier(), server.getRestProbSupplier());
    ...
    Server updateSelfCheckout(Server server) {
        ...
        /**
          ...
          */
        return new SelfCheckout(server.getIdentity(), false, server.getMaxQueue(), newList,
                this.serverAvailableTime,  server.getDefaultIdentity());
    ...
    public String toString() {
        if (!this.server.isSelfCheckout()) {
            ...
            return String.format("%.3f %s waits at self-check %d\n", super.getEventTime(), 
                    super.getCustomer().toString(), this.server.getDefaultIdentity());
        }
    }
}
